...
class VehicleGeometry():
	...
	def getNewPoints(self, x, y, z, yaw, pitch, roll):
		...
		#creating a DCM matrix with the given EUler angles
		DCM = Rotations.euler2DCM(yaw,pitch,roll)

		#Multiply newPoints by DCM to encroporate in to inertial frame
		tempPoints = MatrixMath.multiply(newPoints,DCM)
		
		#offset from matrix library works as matrix add/scale
		tempPoints = MatrixMath.offset(tempPoints, x,y,z)
		#convert xyz to enu

		newPoints = Rotations.ned2enu(tempPoints)
		#need to add displacemtn and should be gucci
		return newPoints
